...
function Toolbar({ view, anchor }: { view: CalendarView; anchor: Date }) {
  ...
  const todayLink =
    view === "month"
      ? "/admin/calendar"
      : `/admin/calendar?view=${view}&date=${ymd(new Date())}`;
  return (
...
function hrefFor(view: CalendarView, d: Date): string {
  if (view === "month") {
    ...
    return `/admin/calendar?month=${d.getUTCFullYear()}-${pad(d.getUTCMonth() + 1)}`;
  }
  return `/admin/calendar?view=${view}&date=${ymd(d)}`;
}
        ...
        {days.map((d) => {
          ...
          return (
            <div key={d.date} className="min-h-[360px] bg-white">
              <Link
                href={`/admin/calendar?view=day&date=${d.date}`}
