/**
 * Refunds (`p_refund`) — list + create.
 * Mirrors /api/orders shape. Each refund optionally links back to a `p_order`.
 *
 * Org-scoping: rows are filtered by `organization_id = session.org_id`, and
 * inserts stamp it from the session. `p_refund` gained the column in 0006.
 */
...
export async function GET(req: Request) {
  ...
  const { search, status, clientId, orderId, page, pageSize, sort, dir } =
    parsed.data;
  ...
  let q = (supabase as any)
    ...
    .select(
      ...
      p_order_id, created_at,
      hotels:p_refund_hotel(id),
      avias:p_refund_avia(id),
      transfers:p_refund_transfer(id),
      excursions:p_refund_excursion(id),
      ensures:p_refund_ensure(id),
      visas:p_refund_visa(id),
      services:p_refund_service(id),
      tourists:p_refund_tourist(id)
    `,
      { count: "exact" },
    )
  ...
  if (orderId) q = q.eq("p_order_id", orderId);
